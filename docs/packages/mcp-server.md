...
`@framingui/mcp-server`는 **MCP Protocol (JSON-RPC 2.0)**을 통해 Claude Code에 16개의 디자인 시스템 도구와 9개의 MCP Prompts를 제공합니다.
...
프로덕션 레디 스크린을 생성하기 위한 워크플로우 도구입니다. 실제 사용 시에는 Step 0에서 스타일 계약을 먼저 확인한 뒤, 아래 4단계를 진행합니다.
...
| Step 4/4 | `validate-environment`          | 프로젝트 환경 검증 (의존성 + Tailwind + 스타일 계약) | `projectPath`, `requiredPackages`  |
...
| 도구                   | 설명                                         | 필수 파라미터                     |
| ---------------------- | -------------------------------------------- | --------------------------------- |
| `validate-environment` | NPM 패키지 + Tailwind CSS + 스타일 계약 검증 | `projectPath`, `requiredPackages` |
...
```
  -> 대상 프로젝트가 framingui-native / host-utility / mixed / unknown 중 무엇인지 확인
  -> utility-first를 유지할지, FramingUI 변수 계약으로 맞출지 결정
```
...
```
  ...
  -> 스타일 계약이 FramingUI 컴포넌트 기본 스타일과 호환되는지 검증
  -> 누락된 패키지의 설치 명령어 제공
```
...
MCP 서버는 모든 MCP 클라이언트에서 사용 가능한 9개의 내장 프롬프트를 제공합니다:
...
**사용 시점:** 프로덕션 스크린 생성, `/screen`, `/section`, `/draft` 워크플로우 명확화 필요 시
...
- `/responsive` 명령 계약
- `mobile-first`, `tablet-safe`, `touch-optimized`
- `--density preserve|denser|lighter`
- `list_tokens -> validate-screen-definition -> generate_screen`

### 4. a11y-workflow

**목적:** 접근성 점검 및 수정 워크플로우

### 5. theme-swap-workflow

**목적:** 동일 구조에 새 테마를 재적용하는 워크플로우

### 6. doctor-workflow

**목적:** 인증, 테마 접근, 환경 설정, 스타일 계약 충돌을 진단하는 워크플로우

### 7. slash-commands

**목적:** FramingUI 슬래시 커맨드 카탈로그
...
**목적:** 특정 슬래시 커맨드의 usage, args, options, examples, workflow를 조회

**예시:** `command=/responsive`

### 9. update-workflow

**목적:** 프로젝트에 설치된 FramingUI 패키지를 한 번에 업데이트하는 워크플로우

**예시:** `/update . --check`

### CLI Adapter Export

클라이언트별 슬래시 커맨드 adapter 템플릿은 CLI로 내보낼 수 있습니다.
...
이 출력은 각 클라이언트에서 command palette, tab-help, fallback help surface를 붙일 때 사용하는 canonical metadata입니다.
...
직접 터미널에서 `npx @framingui/mcp-server` 또는 `framingui-mcp`를 실행하면, interactive TTY 환경에서는 stdio 서버 대신 온보딩 가이드를 출력합니다. MCP 서버를 명시적으로 시작하려면 `framingui-mcp server`를 사용합니다.
...
`checkStyles: true` (기본값)으로 설정하면 다음을 추가로 검증합니다:

- 공통 글로벌 CSS 진입점 존재 여부
- `@framingui/ui/styles` import 존재 여부
- 핵심 FramingUI 변수(`--bg-background`, `--border-default`, `--text-tertiary`) 정의 여부
- 프로젝트 상태 분류:
  - `framingui-native`
  - `host-utility`
  - `mixed`
