...
// ─────────────────────────────────────────────────────────────────────────────
const { mockAuthenticateMcpRequest, mockLoadTheme, mockOklchToCSS } = vi.hoisted(() => ({
  mockAuthenticateMcpRequest: vi.fn(),
  mockLoadTheme: vi.fn(),
  mockOklchToCSS: vi.fn(),
}));
...
const sampleTheme = {
  ...
  tokens: {
    ...
    semantic: {
      background: { canvas: '#fff', surface: {}, elevated: {} },
...
beforeEach(() => {
  vi.clearAllMocks();
  mockOklchToCSS.mockReturnValue('oklch(0.5 0.2 240)');
...
describe('GET /api/mcp/themes/[id]/css', () => {
  ...
  describe('정상 응답', () => {
    ...
    it('CSS에 테마 ID 주석이 포함된다', async () => {
      ...
      expect(data.css).toContain('square-minimalism');
    });
  });
